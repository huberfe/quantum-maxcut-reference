...
Given a graph $$G(V,E,w)$$, with vertices $$V = [n]$$, edges $$E \subseteq V \times V$$, $$m = |E|$$, and weights $$w\in \mathbb{R}_{\geq 0}^{m}$$ and some integer $$1\le k \leq \lfloor n/2\rfloor$$, define the $k$-th token graph of $$G$$ as the graph with
